...
// ChinfluxClusterWebhook implements defaulting and validation.
type ChinfluxClusterWebhook struct{}

func SetupChinfluxClusterWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr, &ChinfluxCluster{}).
		WithDefaulter(&ChinfluxClusterWebhook{}).
		WithValidator(&ChinfluxClusterWebhook{}).
		Complete()
}

// +kubebuilder:webhook:path=/mutate-chinflux-chinflux-io-v1alpha1-chinfluxcluster,mutating=true,failurePolicy=fail,sideEffects=None,groups=chinflux.chinflux.io,resources=chinfluxclusters,verbs=create;update,versions=v1alpha1,name=mchinfluxcluster.kb.io,admissionReviewVersions=v1

func (w *ChinfluxClusterWebhook) Default(_ context.Context, obj *ChinfluxCluster) error {
	if obj.Spec.Replicas == nil {
		obj.Spec.Replicas = ptr.To(int32(1))
	}

	if obj.Spec.Image == "" {
		obj.Spec.Image = "chinflux:latest"
	}
...
// +kubebuilder:webhook:path=/validate-chinflux-chinflux-io-v1alpha1-chinfluxcluster,mutating=false,failurePolicy=fail,sideEffects=None,groups=chinflux.chinflux.io,resources=chinfluxclusters,verbs=create;update,versions=v1alpha1,name=vchinfluxcluster.kb.io,admissionReviewVersions=v1

func (w *ChinfluxClusterWebhook) ValidateCreate(_ context.Context, obj *ChinfluxCluster) (admission.Warnings, error) {
	return validateCluster(obj)
}

func (w *ChinfluxClusterWebhook) ValidateUpdate(_ context.Context, _ *ChinfluxCluster, newObj *ChinfluxCluster) (admission.Warnings, error) {
	return validateCluster(newObj)
}

func (w *ChinfluxClusterWebhook) ValidateDelete(_ context.Context, _ *ChinfluxCluster) (admission.Warnings, error) {
	return nil, nil
}

func validateCluster(cluster *ChinfluxCluster) (admission.Warnings, error) {
	replicas := int32(1)
