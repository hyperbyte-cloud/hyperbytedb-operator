...
kind-create: ## Create a Kind cluster for local development.
	@case "$$($(KIND) get clusters 2>/dev/null)" in \
		*"$(KIND_CLUSTER_NAME)"*) echo "Kind cluster '$(KIND_CLUSTER_NAME)' already exists." ;; \
		*) $(KIND) create cluster --name $(KIND_CLUSTER_NAME) --config hack/kind/kind-config.yaml ;; \
	esac
...
.PHONY: kind-deploy
kind-deploy: manifests kustomize install ## Deploy operator into Kind cluster.
	cd config/manager && "$(KUSTOMIZE)" edit set image controller=$(OPERATOR_IMG)
	"$(KUSTOMIZE)" build config/default | ENABLE_WEBHOOKS=false "$(KUBECTL)" apply -f -

.PHONY: kind-test
kind-test: ## Run e2e tests against the Kind cluster.
	KIND=$(KIND) KIND_CLUSTER=$(KIND_CLUSTER_NAME) go test -tags=e2e ./test/e2e/ -v -ginkgo.v -timeout 10m
