...
var _ = Describe("ChinfluxRestore Controller", func() {
	Context("When restoring with a missing cluster", func() {
		...
		BeforeEach(func() {
			...
			if err != nil && errors.IsNotFound(err) {
				Expect(k8sClient.Create(ctx, &chinfluxv1alpha1.ChinfluxRestore{
					ObjectMeta: metav1.ObjectMeta{Name: restoreName, Namespace: "default"},
					Spec: chinfluxv1alpha1.ChinfluxRestoreSpec{
						ClusterName: "nonexistent-cluster",
						BackupName:  "some-backup",
						Source: &chinfluxv1alpha1.RestoreSource{
		...
		AfterEach(func() {
			resource := &chinfluxv1alpha1.ChinfluxRestore{}
		...
		It("should set status to Failed", func() {
			controllerReconciler := &ChinfluxRestoreReconciler{
				Client:   k8sClient,
			...
			Expect(restore.Status.Phase).To(Equal(chinfluxv1alpha1.RestorePhaseFailed))
		})
	})
	...
	Context("When restoring a valid cluster", func() {
		...
		BeforeEach(func() {
			...
			By("creating the ChinfluxRestore resource")
			restore := &chinfluxv1alpha1.ChinfluxRestore{}
			err = k8sClient.Get(ctx, restoreNN, restore)
			if err != nil && errors.IsNotFound(err) {
				Expect(k8sClient.Create(ctx, &chinfluxv1alpha1.ChinfluxRestore{
		...
		AfterEach(func() {
			for _, nn := range []types.NamespacedName{restoreNN, backupNN, clusterNN} {
				obj := &chinfluxv1alpha1.ChinfluxRestore{}
				if nn == backupNN {
					b := &chinfluxv1alpha1.ChinfluxBackup{}
				...
				if nn == clusterNN {
					c := &chinfluxv1alpha1.ChinfluxCluster{}
		...
		It("should resolve S3 source from backup and begin restore", func() {
			...
			if err != nil {
				...
				Expect(restore.Status.Phase).NotTo(Equal(chinfluxv1alpha1.RestorePhaseFailed))
			}
		})
	})
})
