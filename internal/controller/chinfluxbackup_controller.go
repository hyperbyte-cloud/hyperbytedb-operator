...
// ChinfluxBackupReconciler reconciles a ChinfluxBackup object.
type ChinfluxBackupReconciler struct {
	client.Client
	Scheme   *runtime.Scheme
	Recorder record.EventRecorder
}

// +kubebuilder:rbac:groups=chinflux.chinflux.io,resources=chinfluxbackups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=chinflux.chinflux.io,resources=chinfluxbackups/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=chinflux.chinflux.io,resources=chinfluxbackups/finalizers,verbs=update
// +kubebuilder:rbac:groups=batch,resources=jobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=batch,resources=cronjobs,verbs=get;list;watch;create;update;patch;delete

func (r *ChinfluxBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, types.NamespacedName{
		...
		if apierrors.IsNotFound(err) {
			...
			backup.Status.Phase = chinfluxv1alpha1.BackupPhaseFailed
...
func (r *ChinfluxBackupReconciler) reconcileJob(ctx context.Context, backup *chinfluxv1alpha1.ChinfluxBackup, cluster *chinfluxv1alpha1.ChinfluxCluster) (ctrl.Result, error) {
	jobName := backup.Name

	existing := &batchv1.Job{}
	err := r.Get(ctx, types.NamespacedName{Name: jobName, Namespace: backup.Namespace}, existing)
	if apierrors.IsNotFound(err) {
		if backup.Status.Phase == chinfluxv1alpha1.BackupPhaseCompleted ||
			backup.Status.Phase == chinfluxv1alpha1.BackupPhaseFailed {
			return ctrl.Result{}, nil
		}
		...
		now := metav1.Now()
		backup.Status.Phase = chinfluxv1alpha1.BackupPhaseRunning
	...
	if existing.Status.Succeeded > 0 {
		now := metav1.Now()
		backup.Status.Phase = chinfluxv1alpha1.BackupPhaseCompleted
...
func (r *ChinfluxBackupReconciler) reconcileCronJob(ctx context.Context, backup *chinfluxv1alpha1.ChinfluxBackup, cluster *chinfluxv1alpha1.ChinfluxCluster) (ctrl.Result, error) {
...
func (r *ChinfluxBackupReconciler) buildBackupJob(backup *chinfluxv1alpha1.ChinfluxBackup, cluster *chinfluxv1alpha1.ChinfluxCluster, name string) *batchv1.Job {
	...
	if len(backup.Spec.Databases) > 0 {
		dbFilter = fmt.Sprintf("--databases %s ", strings.Join(backup.Spec.Databases, ","))
	}
...
echo "Starting chinflux backup..."
chinflux backup %s--output "${BACKUP_DIR}"
					...
					Containers: []corev1.Container{
						{
							...
							VolumeMounts: []corev1.VolumeMount{
								{Name: "data", MountPath: "/var/lib/chinflux", ReadOnly: true},
							},
						},
					},
...
func (r *ChinfluxBackupReconciler) buildBackupS3Path(backup *chinfluxv1alpha1.ChinfluxBackup) string {
	s3 := backup.Spec.Destination.S3
	path := s3.Bucket
	if s3.Prefix != "" {
...
func (r *ChinfluxBackupReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&chinfluxv1alpha1.ChinfluxBackup{}).
		Owns(&batchv1.Job{}).
		Owns(&batchv1.CronJob{}).
		Named("chinfluxbackup").
		Complete(r)
}
