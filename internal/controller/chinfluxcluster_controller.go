...
const clusterFinalizer = "chinflux.chinflux.io/finalizer"

// ChinfluxClusterReconciler reconciles a ChinfluxCluster object.
type ChinfluxClusterReconciler struct {
	client.Client
	Scheme   *runtime.Scheme
	Recorder record.EventRecorder
	Members  *chinflux.MemberManager
}

// +kubebuilder:rbac:groups=chinflux.chinflux.io,resources=chinfluxclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=chinflux.chinflux.io,resources=chinfluxclusters/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=chinflux.chinflux.io,resources=chinfluxclusters/finalizers,verbs=update
...
func (r *ChinfluxClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	configHash := chinflux.ConfigHash(cluster)

	prevSTSReplicas := int32(0)
	curSTS := &appsv1.StatefulSet{}
	switch err := r.Get(ctx, types.NamespacedName{
		Name: chinflux.StatefulSetName(cluster), Namespace: cluster.Namespace,
	}, curSTS); {
	...
	// 4. Headless Service
	if err := r.reconcileService(ctx, cluster, chinflux.BuildHeadlessService(cluster)); err != nil {
		return r.setFailedStatus(ctx, cluster, "HeadlessServiceFailed", err)
	}

	// 5. Client Service
	if err := r.reconcileService(ctx, cluster, chinflux.BuildClientService(cluster)); err != nil {
	...
	if stsResult.SpecReplicas != replicas {
		...
		r.updatePhase(ctx, cluster, chinfluxv1alpha1.ClusterPhaseScaling)
		return ctrl.Result{RequeueAfter: 5 * time.Second}, nil
	}

	// 8. Orphan PVC cleanup after scale-down (pods already terminated by StatefulSet)
	if prevSTSReplicas > replicas {
		if err := chinflux.CleanupOrphanPVCs(ctx, r.Client, cluster, prevSTSReplicas, replicas); err != nil {
	...
	cluster.Status.ClusterState = chinflux.DeriveClusterState(memberStatuses)
...
func (r *ChinfluxClusterReconciler) reconcileTLS(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, replicas int32) error {
	...
	secretName := chinflux.TLSSecretName(cluster)
	...
	secret, err := chinflux.BuildSelfSignedTLSSecret(cluster, cluster.Namespace, replicas)
	if err != nil {
...
func (r *ChinfluxClusterReconciler) reconcileConfigMap(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster) error {
	desired := chinflux.BuildConfigMap(cluster)
...
func (r *ChinfluxClusterReconciler) reconcilePeersConfigMap(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, replicas int32) error {
	desired := chinflux.BuildPeersConfigMap(cluster, cluster.Namespace, replicas)
	if err := controllerutil.SetControllerReference(cluster, desired, r.Scheme); err != nil {
		return err
	}
...
func (r *ChinfluxClusterReconciler) reconcileService(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, desired *corev1.Service) error {
	if err := controllerutil.SetControllerReference(cluster, desired, r.Scheme); err != nil {
		return err
	}
...
func (r *ChinfluxClusterReconciler) reconcileStatefulSet(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, configHash string) (stsReconcileResult, error) {
	desired := chinflux.BuildStatefulSet(cluster, configHash)
	if err := controllerutil.SetControllerReference(cluster, desired, r.Scheme); err != nil {
...
func (r *ChinfluxClusterReconciler) runScaleDownClusterHooks(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, prevReplicas, desiredReplicas int32) error {
	if desiredReplicas >= prevReplicas || prevReplicas < 1 {
		return nil
	}

	log := logf.FromContext(ctx)
	port := chinflux.ServerPort(cluster)
	stsName := chinflux.StatefulSetName(cluster)
	headlessSvc := chinflux.HeadlessServiceName(cluster)
...
func (r *ChinfluxClusterReconciler) handleAutoFailover(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, members []chinfluxv1alpha1.MemberStatus) error {
	...
	for _, m := range chinflux.FindUnhealthyMembers(members) {
...
func (r *ChinfluxClusterReconciler) reconcileServiceMonitor(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster) error {
	desired := chinflux.BuildServiceMonitor(cluster)
...
func (r *ChinfluxClusterReconciler) reconcilePDB(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster) error {
	replicas := int32(1)
	if cluster.Spec.Replicas != nil {
		replicas = *cluster.Spec.Replicas
	}
	...
	desired := &policyv1.PodDisruptionBudget{
		...
		Spec: policyv1.PodDisruptionBudgetSpec{
			...
			Selector: &metav1.LabelSelector{
				MatchLabels: chinflux.CommonLabels(cluster),
			},
		},
	}
...
func (r *ChinfluxClusterReconciler) reconcileHPA(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster) error {
	...
	desired := &autoscalingv2.HorizontalPodAutoscaler{
		...
		Spec: autoscalingv2.HorizontalPodAutoscalerSpec{
			ScaleTargetRef: autoscalingv2.CrossVersionObjectReference{
				APIVersion: "apps/v1",
				Kind:       "StatefulSet",
				Name:       chinflux.StatefulSetName(cluster),
			},
...
func (r *ChinfluxClusterReconciler) verifyClusterTopology(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, replicas int32) {
	log := logf.FromContext(ctx)
	port := chinflux.ServerPort(cluster)
	stsName := chinflux.StatefulSetName(cluster)
	headlessSvc := chinflux.HeadlessServiceName(cluster)
...
func (r *ChinfluxClusterReconciler) monitorReplicationHealth(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, replicas int32) {
	log := logf.FromContext(ctx)
	port := chinflux.ServerPort(cluster)
	stsName := chinflux.StatefulSetName(cluster)
	headlessSvc := chinflux.HeadlessServiceName(cluster)
...
func (r *ChinfluxClusterReconciler) updateStatus(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, sts stsReconcileResult, configHash string) {
	...
	if sts.ReadyReplicas == sts.SpecReplicas && sts.SpecReplicas > 0 {
		cluster.Status.Phase = chinfluxv1alpha1.ClusterPhaseRunning
		meta.SetStatusCondition(&cluster.Status.Conditions, metav1.Condition{
			Type:               "Available",
			Status:             metav1.ConditionTrue,
		...
		if cluster.Status.Phase != chinfluxv1alpha1.ClusterPhaseFailed &&
			cluster.Status.Phase != chinfluxv1alpha1.ClusterPhaseScaling &&
			cluster.Status.Phase != chinfluxv1alpha1.ClusterPhaseUpgrading {
			cluster.Status.Phase = chinfluxv1alpha1.ClusterPhaseInitializing
		}
		meta.SetStatusCondition(&cluster.Status.Conditions, metav1.Condition{
			Type:               "Available",
			Status:             metav1.ConditionFalse,
...
func (r *ChinfluxClusterReconciler) updatePhase(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, phase chinfluxv1alpha1.ClusterPhase) {
	cluster.Status.Phase = phase
	_ = r.Status().Update(ctx, cluster)
}

func (r *ChinfluxClusterReconciler) setFailedStatus(ctx context.Context, cluster *chinfluxv1alpha1.ChinfluxCluster, reason string, err error) (ctrl.Result, error) {
	cluster.Status.Phase = chinfluxv1alpha1.ClusterPhaseFailed
	meta.SetStatusCondition(&cluster.Status.Conditions, metav1.Condition{
		Type:               "Degraded",
		Status:             metav1.ConditionTrue,
		Reason:             reason,
...
func (r *ChinfluxClusterReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&chinfluxv1alpha1.ChinfluxCluster{}).
		Owns(&appsv1.StatefulSet{}).
		Owns(&corev1.Service{}).
		Owns(&corev1.ConfigMap{}).
		Owns(&corev1.Secret{}).
		Owns(&policyv1.PodDisruptionBudget{}).
		Named("chinfluxcluster").
		Complete(r)
}
