...
func ConfigMapName(cluster *v1alpha1.ChinfluxCluster) string {
	return cluster.Name + "-config"
}

func BuildConfigMap(cluster *v1alpha1.ChinfluxCluster) *corev1.ConfigMap {
...
func ConfigHash(cluster *v1alpha1.ChinfluxCluster) string {
	h := sha256.Sum256([]byte(renderConfigTOMLWithClusterEnabled(cluster, false)))
	return fmt.Sprintf("%x", h[:8])
}

func renderConfigTOML(cluster *v1alpha1.ChinfluxCluster) string {
	replicas := int32(1)
...
func renderConfigTOMLWithClusterEnabled(cluster *v1alpha1.ChinfluxCluster, clusterEnabled bool) string {
	spec := &cluster.Spec
	...
	if spec.Server.TLS != nil && spec.Server.TLS.Enabled {
		...
		b.WriteString("cert_file = \"/etc/chinflux/tls/tls.crt\"\n")
		b.WriteString("key_file = \"/etc/chinflux/tls/tls.key\"\n")
	}

	b.WriteString("\n[storage]\n")
	b.WriteString(fmt.Sprintf("data_dir = \"/var/lib/chinflux/data\"\n"))
	b.WriteString(fmt.Sprintf("wal_dir = \"/var/lib/chinflux/wal\"\n"))
	b.WriteString(fmt.Sprintf("meta_dir = \"/var/lib/chinflux/meta\"\n"))
	backend := "local"
	...
	b.WriteString("\n[chdb]\n")
	b.WriteString(fmt.Sprintf("session_data_path = \"/var/lib/chinflux/chdb\"\n"))
	poolSize := int32(4)
...
func writeClusterSection(b *strings.Builder, spec *v1alpha1.ChinfluxClusterSpec, clusterEnabled bool) {
	b.WriteString("\n[cluster]\n")
	b.WriteString(fmt.Sprintf("enabled = %t\n", clusterEnabled))
	b.WriteString("replication_log_dir = \"/var/lib/chinflux/replication_log\"\n")
