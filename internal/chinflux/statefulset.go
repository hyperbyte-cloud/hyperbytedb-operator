...
func StatefulSetName(cluster *v1alpha1.ChinfluxCluster) string {
	return cluster.Name
}

// BuildStatefulSet constructs the full StatefulSet for a ChinfluxCluster.
// configHash is a short digest from chinflux.ConfigHash (replica-independent);
// it is placed as a pod annotation so static config edits trigger a rolling restart.
func BuildStatefulSet(cluster *v1alpha1.ChinfluxCluster, configHash string) *appsv1.StatefulSet {
	replicas := int32(1)
...
NODE_ID=$((ORDINAL + 1))
echo "export CHINFLUX__CLUSTER__NODE_ID=$NODE_ID" > /shared/env.sh

# Own address is the entry matching our ordinal in the peers ConfigMap
SELF_ADDR=$(cat /peers/${ORDINAL} 2>/dev/null || echo "")
echo "export CHINFLUX__CLUSTER__CLUSTER_ADDR=${SELF_ADDR}" >> /shared/env.sh
...
echo "export CHINFLUX__CLUSTER__PEERS=${PEERS}" >> /shared/env.sh
`

	entrypoint := `#!/bin/sh
. /shared/env.sh
exec chinflux --config /etc/chinflux/config.toml serve
`
	...
	// --------------- env vars ---------------
	env := []corev1.EnvVar{
		{Name: "CHINFLUX__STORAGE__DATA_DIR", Value: "/var/lib/chinflux/data"},
		{Name: "CHINFLUX__STORAGE__WAL_DIR", Value: "/var/lib/chinflux/wal"},
		{Name: "CHINFLUX__STORAGE__META_DIR", Value: "/var/lib/chinflux/meta"},
		{Name: "CHINFLUX__CHDB__SESSION_DATA_PATH", Value: "/var/lib/chinflux/chdb"},
	}
	...
	if cluster.Spec.Storage.S3 != nil && cluster.Spec.Storage.S3.CredentialsSecretName != "" {
		env = append(env,
			corev1.EnvVar{
				Name: "CHINFLUX__STORAGE__S3__ACCESS_KEY_ID",
			...
			corev1.EnvVar{
				Name: "CHINFLUX__STORAGE__S3__SECRET_ACCESS_KEY",
				ValueFrom: &corev1.EnvVarSource{
					SecretKeyRef: &corev1.SecretKeySelector{
						LocalObjectReference: corev1.LocalObjectReference{Name: cluster.Spec.Storage.S3.CredentialsSecretName},
	...
	if cluster.Spec.Server.TLS != nil && cluster.Spec.Server.TLS.Enabled {
		env = append(env,
			corev1.EnvVar{Name: "CHINFLUX__SERVER__TLS__ENABLED", Value: "true"},
			corev1.EnvVar{Name: "CHINFLUX__SERVER__TLS__CERT_FILE", Value: "/etc/chinflux/tls/tls.crt"},
			corev1.EnvVar{Name: "CHINFLUX__SERVER__TLS__KEY_FILE", Value: "/etc/chinflux/tls/tls.key"},
		)
	}
	...
	volumeMounts := []corev1.VolumeMount{
		...
		{Name: "config", MountPath: "/etc/chinflux", ReadOnly: true},
	...
	if configHash != "" {
		podAnnotations["chinflux.chinflux.io/config-hash"] = configHash
	}
